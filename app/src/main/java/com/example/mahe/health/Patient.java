...
public class Patient extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Doctor1 = FirebaseDatabase.getInstance().getReference().child("Doctor");
        spinnerDataList = new ArrayList<>();
        adapter = new ArrayAdapter<String>(Patient.this, android.R.layout.simple_spinner_dropdown_item, spinnerDataList);
        spinner.setAdapter(adapter);
        spinnerDataList.add(0, "Select the Doctor");
        retriveData();
        spinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                if(parent.getItemAtPosition(position).equals("Select the Doctor")){
                    //do nothing
                    text.setText(" ");
                }
                else{
                            ...
                            .addListenerForSingleValueEvent(new ValueEventListener() {
                                ...
                                public void onDataChange(DataSnapshot dataSnapshot) {
                                    ...
                                    for(DataSnapshot snapshot: dataSnapshot.getChildren()) {
                                        ...
                                        if (name.equals(pass) && (un1.equals(item))) {
                                            s = s + "\n" + "Patient Name :" + snapshot.child("PName").getValue().toString() + "\n" +
                                                    ...
                                                    "\n" + "Diagnosis : " + snapshot.child("DecryptDiag").getValue().toString() + "\n" +
                                                    "\n" + "Prescription : " + snapshot.child("Prescription").getValue().toString() + "\n" +
                                                    "\n" + "Lab Test : " + snapshot.child("Lab Test").getValue().toString() + "\n" + "\n \n";
